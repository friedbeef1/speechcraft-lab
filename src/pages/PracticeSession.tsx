...
import { ChevronLeft, ChevronRight, Mic, Square, X, Check, Loader2, Flag } from "lucide-react";
...
const fallbackPrompts = [{
  text: "Share your thoughts on this topic and why it matters.",
  tips: ["Focus on personal connection", "Use specific examples", "Keep it conversational"]
}, {
  text: "Describe a recent experience related to this subject.",
  tips: ["Share concrete details", "Explain what you learned", "Connect to bigger picture"]
}, {
  text: "Explain your perspective in a way that's easy to understand.",
  tips: ["Use simple language", "Break down complex ideas", "Relate to common experiences"]
}];
const PracticeSession = () => {
  ...
  const processRecordingInBackground = async (base64Audio: string, cardIndex: number, prompt: string, duration: number) => {
    try {
      ...
      if (error) {
        ...
        if (errorMessage.includes('Rate limit exceeded')) {
          ...
          setRecordings(prev => prev.map(r => r.cardIndex === cardIndex ? {
            ...r,
            status: 'error' as const
          } : r));
          toast.error("Rate limit reached! Please sign in for unlimited recordings.", {
            duration: 6000
          });
          return;
        }
  ...
  const handleFinishSession = async () => {
    ...
    if (completedRecordings.length === 0) {
      if (errorRecordings.length > 0) {
        toast.error("All recordings failed. Please sign in to continue or try again later.", {
          duration: 6000
        });
      } else {
    ...
    const validRecordings = completedRecordings.filter(r => {
      const isValid = r.transcript && r.transcript.trim().length > 0 && r.duration && r.duration > 0 && r.fillerWordCount !== undefined;
      if (!isValid) {
    ...
    if (validRecordings.length < completedRecordings.length) {
      toast.warning(`${completedRecordings.length - validRecordings.length} recording(s) had incomplete data and were skipped.`, {
        duration: 4000
      });
    }
    if (errorRecordings.length > 0) {
      toast.info(`Showing results for ${validRecordings.length} completed recording(s). ${errorRecordings.length} failed due to rate limits.`, {
        duration: 5000
      });
    }
  ...
  return <div className="max-w-4xl mx-auto space-y-4 sm:space-y-6 pb-6 sm:pb-8 px-4 sm:px-6 animate-fade-in">
      {/* Header */}
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          {completedCount > 0 && <Badge variant="default" className="text-xs sm:text-sm px-2 sm:px-3 py-1 sm:py-1.5 glass-light text-gray-900 dark:text-white">
              <Check className="h-3 w-3 mr-1" />
              {completedCount} done
            </Badge>}
        </div>
        <div className="flex items-center gap-2">
          <Button onClick={handleFinishSession} className="gap-2 text-xs sm:text-sm" size="sm" disabled={recordings.length === 0}>
      ...
      <Card className="border-2 border-primary/30 shadow-glass-lg hover:shadow-glass-lg transition-smooth relative">
        <CardContent className="py-6 px-4 sm:py-8 sm:px-6 lg:py-12 lg:px-8">
          {getCardStatus(currentCardIndex) === 'completed' && <div className="absolute top-4 right-4 sm:top-6 sm:right-6">
              <Badge variant="default" className="gap-1 glass-light">
                <Check className="h-3 w-3" />
                Done
              </Badge>
            </div>}
          {getCardStatus(currentCardIndex) === 'processing' && <div className="absolute top-4 right-4 sm:top-6 sm:right-6">
      ...
      <div className="flex justify-between items-center gap-2 sm:gap-4">
        <Button variant="outline" onClick={previousCard} disabled={currentCardIndex === 0} className="gap-1 sm:gap-2 text-xs sm:text-sm" size="sm">
          <ChevronLeft className="h-3 w-3 sm:h-4 sm:w-4" />
          <span className="hidden sm:inline">Previous</span>
          <span className="sm:hidden">Prev</span>
        </Button>
        <Button variant="outline" onClick={nextCard} disabled={currentCardIndex === totalCards - 1} className="gap-1 sm:gap-2 text-xs sm:text-sm" size="sm">
      ...
      <div className="flex justify-center py-2 sm:py-4">
        <button onClick={isRecording ? stopRecording : startRecording} className={`w-20 h-20 sm:w-24 sm:h-24 rounded-full flex items-center justify-center transition-smooth shadow-glass-lg hover:scale-110 active:scale-95 ${isRecording ? "bg-gradient-to-br from-red-500 to-red-600 animate-glow-pulse glow-primary" : "bg-gradient-to-br from-primary to-primary/80 glass-light backdrop-blur-xl border-2 border-border glow-primary hover:shadow-glass-lg"}`}>
          {isRecording ? <Square className="h-8 w-8 sm:h-10 sm:w-10 text-primary-foreground fill-primary-foreground" /> : <Mic className="h-8 w-8 sm:h-10 sm:w-10 text-gray-900 dark:text-white" />}
        </button>
      </div>

      {/* Waveform and Timers */}
      {isRecording && <div className="space-y-4 sm:space-y-6 animate-fade-in">
